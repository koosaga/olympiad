...
void init(int s, int e, int p, vector<polyn> &a, vector<pi> &v) {
	if (s == e) {
		tree[p] = a[s];
		sum[p] = v[s][1] - 1;
		coeff[s] = rfact<mint>(v[s][1] - 1) * rfact<mint>(v[s][0] - v[s][1]) * fact<mint>(v[s][0]);
	} else {
		int m = (s + e) / 2;
		init(s, m, 2 * p, a, v);
		init(m + 1, e, 2 * p + 1, a, v);
		tree[p] = tree[2 * p] * tree[2 * p + 1];
		sum[p] = sum[2 * p] + sum[2 * p + 1];
	}
}

void dfs(int s, int e, int p, polyn poly) {
	poly = poly.mod_xk(sum[p] + 1);
	if (s == e) {
		cout << poly[0] * coeff[s] << "\n";
		return;
	}
	int m = (s + e) / 2;
	dfs(s, m, 2 * p, (poly * tree[2 * p + 1]).div_xk(sum[2 * p + 1]));
	dfs(m + 1, e, 2 * p + 1, (poly * tree[2 * p]).div_xk(sum[2 * p]));
}
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < a[i][1]; j++)
			p.push_back(rfact<mint>(j) * rfact<mint>(a[i][0] - j) * fact<mint>(a[i][0]));
		pi[i] = polyn(p);
		sum += a[i][1] - 1;
	}
	init(0, n - 1, 1, pi, a);
	polyn seed;
	{
		vector<mint> p(sum + 1);
		for (int j = 0; j <= sum; j++)
			p[j] = fact<mint>(m - j - 1) * fact<mint>(j);
		reverse(all(p));
		seed = p;
	}
	dfs(0, n - 1, 1, seed);
