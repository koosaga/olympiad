// O((n + q) log n) solution for WF 2021 B
...
template <typename top_tree_node> struct top_tree_node_base {
  ...
  public:
	...
	friend top_tree_node *get_lca(top_tree_node *a, top_tree_node *b) {
		b->expose();
		a->expose();
		top_tree_node *ret = b;
		b->downdate_all();
...
struct node : public top_tree_node_base<node> {
	...
	// values
	lint max_dist[2][2] = {{0, 0}, {0, 0}};
	lint edge_sum = 0;

	void do_flip_path() {
		assert(is_path);
		std::swap(c[0], c[1]);
		swap(max_dist[0][0], max_dist[0][1]);
		swap(max_dist[1][0], max_dist[1][1]);
		lazy_flip_path ^= 1;
	}

	void do_invert_weight() {
		assert(is_path);
		swap(max_dist[0][0], max_dist[1][0]);
		swap(max_dist[0][1], max_dist[1][1]);
		invert_weight ^= 1;
		edge_sum *= -1;
		weight *= -1;
	}
	...
	void downdate() {
		...
		if (invert_weight) {
			assert(is_path);
			if (!is_vert) {
				c[0]->do_invert_weight();
				c[1]->do_invert_weight();
			}
			invert_weight = false;
		}
	}
	...
	void update() {
		...
		if (is_path && !is_vert) {
			for (int j = 0; j < 2; j++) {
				max_dist[j][0] = max(c[0]->max_dist[j][0], (j == 1 ? -1 : 1) * (c[0]->edge_sum + weight) + c[1]->max_dist[j][0]);
				max_dist[j][1] = max(c[1]->max_dist[j][1], (j == 1 ? -1 : 1) * (c[1]->edge_sum + weight) + c[0]->max_dist[j][1]);
			}
			edge_sum += c[0]->edge_sum + c[1]->edge_sum;
		} else {
			for (int j = 0; j < 2; j++) {
				for (int i = 0; i < 3; i++) {
					if (c[i]) {
						max_dist[j][0] = max(max_dist[j][0], c[i]->max_dist[0][0] + (i == 2 ? weight : 0));
					}
				}
				max_dist[j][1] = max_dist[j][0];
			}
		}
	}
};
...
int main() {
	...
	while (q--) {
		...
		T--;
		ptr[S]->make_root();
		node *l = get_lca(ptr[K], ptr[T]);
		if (l->idx == T) {
			cout << "impossible\n";
			continue;
		}
		node *g = get_path(ptr[K], ptr[l->idx]);
		g->downdate_all();
		g->do_invert_weight();
		g->update_all();

		ptr[S]->make_root();
		cout << sum - ptr[S]->max_dist[0][0] << "\n";

		g = get_path(ptr[K], ptr[l->idx]);
		g->downdate_all();
		g->do_invert_weight();
		g->update_all();
	}
