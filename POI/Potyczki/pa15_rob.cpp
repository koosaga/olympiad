...
void solve(vector<pi> congs, int M, int K){
	BigInt m = BigInt(1), r = BigInt(0);
	// N^2 bigint op = N^3?
	for(auto &[mt, rt] : congs){
		for(int j = 0; j < mt; j++){
			if((m * j + r) % mt == rt){
				r += m * j;
				m *= mt;
...
int main(){
	...
	while(r--){
		int x; cin >> x; adj[0][x] = 1;
	}
	n++;
	// small answer.
	{
		word vis = 1;
		for(int i = 0; i < 80808; i++){
			word nxt = 0;
			for(int j = 0; j < n; j++){
				if(vis[j]) nxt |= adj[j];
			}
	...
	vector<word> hasCycle(n);
	for(int i = 0; i < n; i++){
		vector<word> dp(n);
		dp[0].set(i);
		for(int j = 1; j < n; j++){
			for(int k = 0; k < n; k++){
				if(dp[j - 1][k]) dp[j] |= adj[k];
			}
			if(dp[j][i]) hasCycle[j][i] = 1;
		}
	}
	vector<word> badMod(n);
	for(int i = 1; i < n; i++){
		// dp(inCycle, modulo, vertex)
		vector<word> dp[2];
		for(int j = 0; j < 2; j++){
			dp[j].resize(i);
		}
		queue<node> que;
		auto enq = [&](int ic, int md, int v){
			dp[ic][md][v] = 1;
			que.push({ic, md, v});
		};
		enq(0, i - 1, 0);
		while(sz(que)){
			auto x = que.front();
			que.pop();
			if(x.inCycle == 0 && hasCycle[i][x.v] && !dp[1][x.mod][x.v]){
				enq(1, x.mod, x.v);
			}
			word toProp = adj[x.v] & ~dp[x.inCycle][(x.mod + 1) % i];
			for(int j = toProp._Find_first(); j < toProp.size(); j = toProp._Find_next(j)){
				enq(x.inCycle, (x.mod + 1) % i, j);
			}
		}
		for(int j = b + 1; j < n; j++){
			for(int k = 0; k < i; k++){
				if(dp[1][k][j]) badMod[i][k] = 1;
			}
		}
	}
	vector<int> promote(201);
	promote[1] = 1;
	for(int i = 2; i <= 200; i++){
		bool isprime = 1;
		for(int j = 2; j * j <= i; j++){
		...
		if(isprime){
			int goal = i;
			while(goal * i <= 200) goal *= i;
			for(int k = i; k <= 200; k *= i) promote[k] = goal;
		}
	}
	for(int t = 0; t < T; t++){
		vector<word> chk(201);
		for(int i = 1; i < n; i++){
			int period = i / gcd(i, T);
			period = promote[period];
			for(int j = 0; j < period; j++){
				int cur = T * j + t;
				if(badMod[i][cur % i]) chk[period][j] = 1;
			}
		}
		vector<pi> congs;
		bool fucked = false;
		for(int i = 1; i < 201; i++){
			if(promote[i] != i) continue;
			bool ok = 0;
			for(int j = 0; j < i; j++){
				if(!chk[i][j]){
					congs.emplace_back(i, j);
					ok = 1;
					break;
				}
			}
