...
struct node {
	...
	node operator+(const node &nd) const { return node{max(nd.val, val + nd.sum), sum + nd.sum, -1}; }
};

// (opt, sum)
auto _TT = [](node a, node b) -> node { return a + b; };
auto _UU = [](bool a, bool b) -> bool { return a + b; };
auto _UT = [](bool a, node b) -> node { return b; };
auto _FlipT = [](node a) -> node { return a; };
vector<pi> in_which;
vector<vector<int>> gph;
vector<int> dep, din, dout, root;
vector<vector<pi>> req;
int piv;
...
auto treap = treap_base<true, false, false, false, node, bool, decltype(_TT), decltype(_UU), decltype(_UT), decltype(_FlipT)>(_TT, node{-int(1e9), 0, 0}, _UU, 0, _UT, _FlipT);

int rt[696969];
vector<vector<node>> del;
int p;
...
int solve(int x) {
	...
	for (auto &y : gph[x]) {
		ans = max(ans, solve(y));
		rt[x] = treap.unite_by_key(rt[x], rt[y]);
	}
	for (auto &val : del[x]) {
		rt[x] = treap.erase_by_key(rt[x], val);
		assert(rt[x] != -2);
	}
	for (auto &[val, dies] : req[x]) {
		node put = {val + 1, 1, p++};
		rt[x] = treap.insert_by_key(rt[x], put);
		del[dies].push_back(put);
	}
	int cur = max(dep[x], treap.query(rt[x], 0, treap.size(rt[x])).val);
	return max(ans, cur - dep[x]);
}
...
struct cycle_instance {
	...
	int ssolve() {
		...
		for (auto &[l, r, p] : requests) {
			if (l < r) {
				req[l].push_back({p + dep[l], r});
			} else {
				req[l].push_back({p + dep[l], n});
				if (r > 0)
					req[0].push_back({p + dep[l] + dep[0], r});
			}
		}
		return solve(n);
	}
...
int main() {
	...
	}
	{
		...
		for (int i = 0; i < n; i++) {
			if (indeg[i]) {
				if (in_which[i][0] == -1) {
					vector<int> pth;
					for (int j = i; in_which[j][0] == -1; j = p[j]) {
						pth.push_back(j);
						in_which[j] = pi{sz(tc), sz(pth) - 1};
					}
					tc.push_back({sz(pth), vector<array<int, 3>>()});
				}
				root[i] = i;
				dfs(i);
			}
	...
	for (int i = 0; i < m; i++) {
		...
		y--;
		// not in same scc
		if (in_which[root[x]][0] != in_which[root[y]][0]) {
			cout << "-1\n";
			return 0;
		}
		// need to take cycle
		if (in_which[root[x]][1] != in_which[root[y]][1]) {
			if (root[y] != y) {
				cout << "-1\n";
				return 0;
			}
			if (x != root[x])
				req[x].push_back({dep[x], root[x]});
			tc[in_which[root[x]][0]].requests.push_back({in_which[root[x]][1], in_which[root[y]][1], dep[x]});
		} else {
			if (!(din[y] <= din[x] && dout[x] <= dout[y])) {
				cout << "-1\n";
				return 0;
			}
			if (dep[x] > dep[y])
				req[x].push_back({dep[x], y});
		}
	}
	int ans = 0;
	for (int i = 0; i < n; i++) {
