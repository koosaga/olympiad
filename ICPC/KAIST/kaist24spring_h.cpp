...
struct node : public top_tree_node_base<node> {
	...
	// aggregates
	lint cont_max;		// max that contains the vertex or path
	lint tot_max;		// real max
	lint anchor_max[2]; // for path: max that contains each endpoints

	void do_flip_path() {
		assert(is_path);
		std::swap(c[0], c[1]);
		swap(anchor_max[0], anchor_max[1]);
		lazy_flip_path ^= 1;
	}
	...
	void update() {
		if (is_path && !is_vert) {
			cont_max = c[0]->cont_max + c[1]->cont_max;
			tot_max = max({c[0]->tot_max, c[1]->tot_max, c[0]->anchor_max[1] + c[1]->anchor_max[0]});
			anchor_max[0] = max(c[0]->anchor_max[0], c[0]->cont_max + c[1]->anchor_max[0]);
			anchor_max[1] = max(c[1]->anchor_max[1], c[1]->cont_max + c[0]->anchor_max[1]);
		} else {
			if (!is_vert) {
				cont_max = 0;
				tot_max = -1e18;
				if (c[2]) {
					cont_max += max(c[2]->anchor_max[0], 0ll);
					tot_max = max(tot_max, c[2]->tot_max);
				}
				for (int i = 0; i < 2; i++) {
					if (c[i]) {
						cont_max += c[i]->cont_max;
						tot_max = max(tot_max, c[i]->tot_max);
					}
				}
			} else {
				cont_max = tot_max = anchor_max[0] = anchor_max[1] = val;
				for (int i = 0; i < 2; i++) {
					if (c[i]) {
						cont_max += c[i]->cont_max;
						tot_max = max(tot_max, c[i]->tot_max);
				...
				tot_max = max(tot_max, cont_max);
				anchor_max[0] = anchor_max[1] = cont_max;
			}
		}
	}
};
