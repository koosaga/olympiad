	...
	void update() {
		...
		if (is_path) {
			if (is_vert) {
				...
				vsum = c[0]->vsum + c[1]->vsum;
				is = min(c[0]->is, c[1]->is);
				ie = max(c[0]->ie, c[1]->ie);
			}
		} else {
		}
	}
...
int main() {
	...
	vector<node *> vtx(n + 1), edge(n + 1);
	for (int i = 1; i < n; i++) {
		lint sum = 0;
		for (int j = i; j < n - 1 && j < i + 64; j++) {
			sum += a[j];
	...
	for (int i = n; i >= 0; i--) {
		vtx[i] = new node();
		vtx[i]->is_path = vtx[i]->is_vert = true;
		vtx[i]->idx = i;
		vtx[i]->update();
		edge[i] = new node();
		if (nxt[i] > i) {
			link(edge[i], vtx[nxt[i]], vtx[i]);
		}
	}
	auto update = [&](int p) {
		for (int i = p + 1; i >= max(p - 40, 1); i--) {
			...
			if (i < n)
				nxt[i - 1] = min(nxt[i - 1], nxt[i]);
			int upd = nxt[i - 1];
			if (cur != upd) {
				cut(edge[i - 1]);
				link(edge[i - 1], vtx[nxt[i - 1]], vtx[i - 1]);
			}
		}
	};
	auto f = [&](int l, int r) {
	...
	for (int i = 0; i < q; i++) {
		...
		int ml = move_fwd(l), mr = move_bwd(r);
		cout << max({2, f(l, r) * 2 + 1, f(ml, r) * 2 + 2, f(l, mr) * 2 + 2, f(ml, mr) * 2 + 3}) << "\n";
	}
}