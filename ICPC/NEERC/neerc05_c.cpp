...
int dfn[20005], low[20005], piv, dep[20005];
int vis[20005], parent[20005];
InfInt ret = 1;
bool cact = 1;
    ...
    for (int i=0; i<graph[x].size(); i++) {
        if(graph[x][i] != pa){
            if(dfn[graph[x][i]]){
                ...
                ret *= max(dep[x] - dep[graph[x][i]] + 2,1);
                low[x] = min(low[x],dfn[graph[x][i]]);
            }
            else{
                dep[graph[x][i]] = dep[x] + 1;
                low[x] = min(low[x],dfs(graph[x][i],x));
            }
        }
    }
    return low[x];
