...
template <class T> T pick(T x, T y, double p = 0.5) {
...
int main() {
    ...
    array<array<char, mx + 1>, mxd> fact{};
    fact[0][0] = 1;
    for (auto x = 0; x < mx; ++x) {
    ...
    y_combinator([&](auto self, int u, int l, int r) -> void {
        ...
        y_combinator([&](auto self2, int vl, int vr, int l, int r) -> void {
            ...
            int vm = vl + vr >> 1;
            int m = partition(pool.begin() + l, pool.begin() + r, [&](int x) { return fact[q[u]][x] < vm; }) - pool.begin();
            self2(vl, vm, l, m);
            self2(vm, vr, m, r);
        })(0, 10, l, r);
    })(0, 0, (int)pool.size());
