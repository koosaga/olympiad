...
struct BigInt {
	...
	template <typename L, typename R> typename std::enable_if<std::is_convertible<L, BigInt>::value && std::is_convertible<R, BigInt>::value && std::is_lvalue_reference<R &&>::value, BigInt>::type friend operator+(L &&l, R &&r) {
	...
	template <typename L, typename R> typename std::enable_if<std::is_convertible<L, BigInt>::value && std::is_convertible<R, BigInt>::value, BigInt>::type friend operator-(L &&l, R &&r) {
		BigInt result(std::forward<L>(l));
		result -= r;
		return result;
	}
...
const int MAXN = 2005;
BigInt x[MAXN], y[MAXN], p2[MAXN];
BigInt sx[MAXN], ex[MAXN], sy[MAXN], ey[MAXN];

vector<BigInt> solve(int n, BigInt &x, BigInt &y) {
	if (x < -sx[n] || x > ex[n] || y < -sy[n] || y > ey[n]) {
	...
	auto l = solve(n - 1, x, y);
	// x_n + y' == x
	// y_n - x' == y
	BigInt x1 = ::y[n] - y, y1 = x - ::x[n];
	auto r = solve(n - 1, x1, y1);
	reverse(all(r));
	for (auto &z : r) {
		z = p2[n] - z;
		if (sz(l) && l.back() == z)
...
int main() {
	...
	for (int i = 2; i <= n; i++) {
		...
		x[i] = x[i - 1] - y[i - 1];
		y[i] = x[i - 1] + y[i - 1];
		ex[i] = max(ex[i - 1], ey[i - 1] + x[i]);
		ey[i] = max(ey[i - 1], sx[i - 1] + y[i]);
		sx[i] = max(sx[i - 1], sy[i - 1] - x[i]);
		sy[i] = max(sy[i - 1], ex[i - 1] - y[i]);
	}
	for (int i = 0; i < m; i++) {
		int x, y;
		cin >> x >> y;
