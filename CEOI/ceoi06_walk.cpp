...
int main() {
	...
	n += 2;
	vector<int> ja;
	sort(a, a + n, [&](const rect &a, const rect &b) { return a.ex < b.ex; });
	for (int i = 0; i < n; i++) {
	...
	ja.resize(unique(ja.begin(), ja.end()) - ja.begin());
	...
	int q = 0;
	auto locate = [&](int x) {
		int ret = lower_bound(ja.begin(), ja.end(), x) - ja.begin();
		return ret;
	};
	for (int i = 0; i < pnt.size(); i++) {
		...
		while (q < n && a[q].ex <= pnt[i].first) {
			seg.set(locate(a[q].sy) + 1, locate(a[q].ey) - 1, q);
			q++;
		}
		int p = seg.query(locate(pnt[i].second));
