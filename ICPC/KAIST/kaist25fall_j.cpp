	...
	void update() {
		if (is_path) {
			if (is_vert) {
				...
				WS = c[0]->WS + c[1]->WS + W;
				RS = c[0]->RS + c[1]->RS + R;
				comm[0] = c[0]->comm[0] + c[1]->comm[0] + (W + c[0]->WS) * (R + c[1]->RS) * mint(2) - mint(1);
				comm[1] = c[1]->comm[1] + c[0]->comm[1] + (W + c[1]->WS) * (R + c[0]->RS) * mint(2) - mint(1);
			}
		} else {
			WS = W;
			for (int i = 0; i < 3; i++) {
...
int main() {
	...
	for (int i = 0; i < n - 1; i++) {
		...
		new_edge->W = mint(x).inv();
		new_edge->R = mint(x);
		new_edge->update();
		link(new_edge, V[s], V[e]);
	...
	while (q--) {
		...
		if (t == 1) {
			...
			cin >> i >> w;
			i--;
			auto gp = get_path(V[pairs[i][0]], V[pairs[i][1]]);
			gp->downdate_all();
			gp->W = mint(w).inv();
			gp->R = mint(w);
			gp->downdate();
