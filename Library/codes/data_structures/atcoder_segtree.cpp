...
struct node {
	int sum, smin, smax, len;
	node() { sum = smin = smax = len = 0; }
	node(int x) {
		sum = x;
		smin = min(x, 0);
		smax = max(x, 0);
	...
	node operator+(const node &nd) {
		...
		ret.sum = sum + nd.sum;
		ret.len = len + nd.len;
		ret.smin = min(nd.smin, nd.sum + smin);
		ret.smax = max(nd.smax, nd.sum + smax);
		return ret;
	}
};
...
node ET() { return node(); }
int EU() { return 0; }
bool pred(node v) { return v.smin >= 0; }
...
int main() {
	...
	string s;
	cin >> s;
	atcoder::lazy_segtree<node, TT, ET, int, UT, UU, EU> seg[2];
	seg[0] = atcoder::lazy_segtree<node, TT, ET, int, UT, UU, EU>(n);
	seg[1] = atcoder::lazy_segtree<node, TT, ET, int, UT, UU, EU>(n);
	for (int i = 0; i < n; i++) {
		seg[0].set(i, node(s[i] == 'C' ? 1 : -1));
		seg[1].set(i, node(s[i] == 'P' ? 1 : -1));
	}
	while (q--) {
		int t, x;
		cin >> t >> x;
		...
		if (t == 0) {
			s[x] = 'C' + 'P' - s[x];
			seg[0].set(x, node(s[x] == 'C' ? 1 : -1));
			seg[1].set(x, node(s[x] == 'P' ? 1 : -1));
		}
		if (t == 1) {
			int i = (s[x] == 'C' ? 0 : 1);
			int j = seg[i].min_left<pred>(x);
			auto q = seg[i].prod(j, x);
			cout << q.smax + 1 << "\n";
		}
	}
}