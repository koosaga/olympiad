...
template <typename T> struct poly {
	...
	poly operator/=(const T &x) { return *this *= (T(1) / T(x)); }
	...
	poly operator/=(const poly &b) {
		if (deg() < b.deg())
			return *this = poly();
		if (min(deg(), b.deg()) < 256)
			return *this = divmod_slow(b).first;
		int k = deg() - b.deg() + 1;
	...
	poly operator%=(const poly &b) {
		if (deg() < b.deg())
			return *this;
		if (min(deg(), b.deg()) < 256)
			return *this = divmod_slow(b).second;
		poly foo = poly(a);
		foo /= b;
		foo *= b;
		*this = poly(*this) -= foo;
	...
	poly power(int n, int k) {
		if (sz(a) == 0)
			return poly();
		if (k == 0)
			return poly(T(1)).trim(n);
		if (k == 1)
			return trim(n);
		int ptr = 0;
		while (ptr < sz(a) && a[ptr] == T(0))
			ptr++;
		if (1ll * ptr * k >= n)
			return poly();
		n -= ptr * k;
	...
	poly root(int n, int k = 2) {
		...
		for (int i = 1; i < n; i <<= 1) {
			if (k == 2)
				q += trim(2 * i) * q.inv(2 * i);
			else
				q = q * T(k - 1) + trim(2 * i) * q.inv(2 * i).power(k - 1, 2 * i);
			q = q.trim(2 * i) / T(k);
		}
		return q.trim(n);
	}
...
int main() {
	...
	lint m, k;
	cin >> m >> k;
	vector<mint> fact(m + 1), invf(m + 1);
	fact[0] = invf[0] = 1;
	for (int i = 1; i <= m; i++) {
		fact[i] = fact[i - 1] * mint(i);
		invf[i] = mint(1) / fact[i];
	}
	auto bino = [&](int x, int y) { return fact[x] * invf[y] * invf[x - y]; };
	vector<mint> a(k + 1), dp(m + 1);
	for (int i = 0; i <= k; i++) {
		cin >> a[i].val;
	}
	if (k == 0) {
		for (lint n = 1; n <= m; n++) {
			dp[n] = ipow(a[0], n * (n - 1) / 2);
		}
	}
	if (k == 1) {
		dp[0] = 1;
		vector<mint> func(m + 1);
		for (lint i = 1; i <= m; i++) {
			func[i] = invf[i] * ipow(mint(i), i - 2) * ipow(a[1] / a[0], 1ll * i * (i - 1) / 2);
		}
		auto ex = poly(func).exp(m + 5);
		for (int i = 1; i <= m; i++) {
			dp[i] = ex[i];
		}
	}
	if (k >= 2) {
		vector<mint> func(m + 1);
		for (lint i = 1; i <= m; i++) {
			func[i] = invf[i] * ipow(mint(i), i - 1) * ipow(a[1] / a[2], 1ll * i * (i - 1) / 2);
		}
		auto Q = poly(func);
		auto P = (pol(1) - Q).ln(m + 1) + Q + (Q * Q) * pol(mint(1) / mint(2));
		P *= pol(mint(-1) / mint(2));
		for (int j = 1; j <= m; j++) {
			mint func1 = P[j] * mint(j) * ipow(a[2] / a[0], 1ll * j * (j - 1) / 2);
			mint func2 = invf[j] * ipow(mint(j), j - 1) * ipow(a[1] / a[0], 1ll * j * (j - 1) / 2);
			func[j] = func1 + func2;
			func[j] /= mint(j);
		}
		auto ex = poly(func).exp(m + 5);
	...
	for (int i = 2; i <= m; i++) {
		if (k == 1) {
			dp[i] *= ipow(a[0], 1ll * i * (i - 1) / 2) * fact[i];
		}
		if (k >= 2) {
			dp[i] *= ipow(a[0], 1ll * i * (i - 1) / 2) * fact[i];
		}
