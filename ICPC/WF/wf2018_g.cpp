...
pair<Point, double> mCC(Point a, Point b, Point c){
	b -= a; c -= a;
	double d = 2*(conj(b)*c).imag(); if(abs(d)<eps) return make_pair(Point(-1, -1),-1e9);
...
int main(){
	...
	for(auto [x, y, z] : dt){
		...
		Point nj;
		double dist;
		tie(nj, dist) = mCC(poly[xp], poly[yp], poly[zp]);
		Point far(6942069, 42069420);
		int cnt = 0;
		for(int i=0; i<n; i++){
