...
// Deque Palindrome Tree
// Credit: Nachia @ https://judge.yosupo.jp/submission/211751

template <class Elem> struct AmortizedDeque {
  private:
	std::vector<Elem> l;
	std::vector<Elem> r;
  ...
  public:
	...
	Elem &operator[](std::size_t i) { return i < l.size() ? l[l.size() - 1 - i] : r[i - l.size()]; }
	const Elem &operator[](std::size_t i) const { return i < l.size() ? l[l.size() - 1 - i] : r[i - l.size()]; }
	Elem &at(std::size_t i) { return i < l.size() ? l.at(l.size() - 1 - i) : r.at(i - l.size()); }
	const Elem &at(std::size_t i) const { return i < l.size() ? l.at(l.size() - 1 - i) : r.at(i - l.size()); }
...
template <class Char = int> struct DequePalindromicTree {
  private:
	struct Node {
		Node *parent;
		Node *link;
	...
	Node *backAppendable(Char c, Node *p) {
		auto n = int(deq.size());
		while (true) {
			if (p->len == -1 || (p->len < n && deq[n - p->len - 1].ch == c))
				return p;
			auto q = p->link;
			if (q->len == -1 || deq[n - q->len - 1].ch == c)
				return q;
			p = p->quick;
		}
	}
	Node *frontAppendable(Char c, Node *p) {
		auto n = int(deq.size());
		while (true) {
			if (p->len == -1 || (p->len < n && deq[p->len].ch == c))
				return p;
			auto q = p->link;
			if (q->len == -1 || deq[q->len].ch == c)
				return q;
			p = p->quick;
		}
	}
  ...
  public:
	...
	void pushBack(Char ch) {
		...
		if (vit == par->next.end()) {
			...
			v->len = par->len + 2;
			if (par != Odd.get())
				w = backAppendable(ch, par->link)->next[ch];
			v->link = w;
			w->linkcnt += 1;
	...
	void pushFront(Char ch) {
		...
		if (vit == par->next.end()) {
			...
			v->len = par->len + 2;
			if (par != Odd.get())
				w = frontAppendable(ch, par->link)->next[ch];
			v->link = w;
			w->linkcnt += 1;
...
int main() {
	...
	while (q--) {
		...
		cout << dq.numDistinctPalindromes() << " " << dq.longestPrefixPalindrome() << " " << dq.longestSuffixPalindrome() << "\n";
	}
}