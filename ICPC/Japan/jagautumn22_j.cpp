...
template <class T> T det3(T a1, T a2, T a3, T b1, T b2, T b3, T c1, T c2, T c3) { return a1 * (b2 * c3 - c2 * b3) - a2 * (b1 * c3 - c1 * b3) + a3 * (b1 * c2 - c1 * b2); }

bool in_circle(pt a, pt b, pt c, pt d) {
    long double det = -det3<long double>(b.x, b.y, b.sqrLength(), c.x, c.y, c.sqrLength(), d.x, d.y, d.sqrLength());
    det += det3<long double>(a.x, a.y, a.sqrLength(), c.x, c.y, c.sqrLength(), d.x, d.y, d.sqrLength());
    det -= det3<long double>(a.x, a.y, a.sqrLength(), b.x, b.y, b.sqrLength(), d.x, d.y, d.sqrLength());
    det += det3<long double>(a.x, a.y, a.sqrLength(), b.x, b.y, b.sqrLength(), c.x, c.y, c.sqrLength());
    if (fabs(det) > 1e18)
        return det > 0;
    else {

        ll det = -det3<ll>(b.x, b.y, b.sqrLength(), c.x, c.y, c.sqrLength(), d.x, d.y, d.sqrLength());
        det += det3<ll>(a.x, a.y, a.sqrLength(), c.x, c.y, c.sqrLength(), d.x, d.y, d.sqrLength());
        det -= det3<ll>(a.x, a.y, a.sqrLength(), b.x, b.y, b.sqrLength(), d.x, d.y, d.sqrLength());
        det += det3<ll>(a.x, a.y, a.sqrLength(), b.x, b.y, b.sqrLength(), c.x, c.y, c.sqrLength());
        return (det > 0);
    }
}
...
pair<Point, double> mCC(Point a, Point b, Point c) {
    b -= a;
    c -= a;
    double d = 2 * (conj(b) * c).imag();
    if (abs(d) < eps)
        return make_pair(Point(-1, -1), -1e9);
    Point ans = (c * norm(b) - b * norm(c)) * Point(0, -1) / d;
    return make_pair(ans + a, abs(ans));
}
...
bool cross(Point a, Point b, Point c, Point d) {
    bool bh1 = ccw(a, b, c) > 0;
    bool bh2 = ccw(a, b, d) > 0;
    bool bh3 = ccw(c, d, a) > 0;
    bool bh4 = ccw(c, d, b) > 0;
    return bh1 != bh2 && bh3 != bh4;
}
...
long double ccw(pi a, pi b, pi c) {
    ...
    auto x2 = c.first - a.first;
    auto y2 = c.second - a.second;
    return x1 * y2 - y1 * x2;
}
struct line {
    long double a, b, c;
...
HPI::line make_line(Point x, Point y) {
    auto dir = (y - x) * Point(0, 1);
    HPI::line p = {dir.real(), dir.imag(), -1};
    p.c = x.real() * p.a + x.imag() * p.b;
    return p;
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (auto &j : gph[i]) {
            ...
            Point z = (poly[i] + poly[j]) / 2.0;
            l.c = l.a * z.real() + l.b * z.imag();
            v.push_back(l);
        }
        v.push_back({0, 1, 1.0 * h});
        v.push_back({0, -1, 0});
        v.push_back({-1, 0, 0});
        v.push_back({1, 0, 1.0 * w});
        vector<HPI::pi> ans;
        if (!HPI::solve(v, ans))
            continue;
        ...
        for (int j = 0; j < sz(ans); j++) {
            ...
            double dist = abs(Point(ans[j].first, ans[j].second) - poly[i]);
