...
lint solve(int n, pi src, pi dst, vector<pi> p1, vector<pi> p2, vector<int> w){
	...
	for(int i=0; i<n; i++){
		...
		if(p2[i].first < dst.first){
			event.push_back({p2[i].first, p1[i].second, p2[i].second, 1ll * (p2[i].first - p1[i].first) * (w[i] - 100)});
		}
	}
	...
	auto upload = [&](int pos, lint val){
		...
		seg2.upd(pos, 0, v.size() - 1, 1, val - 100ll * v[pos]);
		seg3.upd(pos, 0, v.size() - 1, 1, val + 100ll * v[pos]);
	};
	upload(src.second, 0);
	lint ret = 1e18;
	for(auto &i : event){
	...
	if(leftest_good <= dst.second && dst.second <= rightest_good){
		for(int i=leftest_good; i<=rightest_good; i++){
			ret = min(ret, dist[i] + 100ll * abs(v[i] - v[dst.second]));
		}
	}
	return ret + 100ll * (dst.first - src.first);
...
lint shortest_path(pi src, pi dst, vector<pi> p1, vector<pi> p2, vector<int> w){
	...
	if(ret > 1e13) ret = 100ll * abs(src.first - dst.first) + 100ll * abs(src.second - dst.second);
	return ret;
}
