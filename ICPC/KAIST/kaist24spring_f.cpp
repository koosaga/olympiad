...
struct info {
	lint far[2]; // two far distance
	info() { far[0] = far[1] = -1e18; }
	info(lint x) {
		far[0] = x;
		far[1] = -1e18;
	}
	...
	info extend(lint x) { return info(far[0] + x, far[1] + x); }
	info operator+(const info &i) const {
		if (far[0] <= i.far[0]) {
			info ans = i;
			ans.far[1] = max(i.far[1], far[0]);
			return ans;
		} else {
			info ans = *this;
			ans.far[1] = max(i.far[0], far[1]);
			return ans;
		}
	}
};

// ALL SHITS ARE EDGE BASED
struct node : public top_tree_node_base<node> {
	...
	// farthest
	lint len = 0;	  // path size
	info far_info[2]; // two farthest for each projection (other only for paths)
	info just_far;	  // just two farthest (only for paths)
	lint diam_path;	  // real diam
	lint diam_just;	  // real diam, but not doing path

	void do_flip_path() {
		assert(is_path);
		std::swap(c[0], c[1]);
		swap(far_info[0], far_info[1]);
		lazy_flip_path ^= 1;
	}
	...
	void update() {
		if (is_path && !is_vert) {
			far_info[0] = c[0]->far_info[0] + c[1]->far_info[0].extend(c[0]->len + elen);
			far_info[1] = c[1]->far_info[1] + c[0]->far_info[1].extend(c[1]->len + elen);
			len = c[0]->len + c[1]->len + elen;
			just_far = c[0]->just_far + c[1]->just_far;
			diam_just = max(c[0]->diam_just, c[1]->diam_just);
			diam_path = max({c[0]->diam_path, c[1]->diam_path, c[0]->far_info[1].far[0] + c[1]->far_info[0].far[0] + elen});
		} else {
			len = 0;
			if (!is_vert) {
				far_info[0] = info();
				diam_path = 0;
				if (c[2]) {
					far_info[0] = c[2]->far_info[0].extend(elen);
					diam_path = c[2]->diam_path;
				}
				for (int i = 0; i < 2; i++) {
					if (c[i]) {
						diam_path = max(diam_path, c[i]->far_info[0].far[0] + far_info[0].far[0]);
						diam_path = max(diam_path, c[i]->diam_path);
						far_info[0] = far_info[0] + c[i]->far_info[0];
					}
				}
				diam_path = max(diam_path, far_info[0].far[0]);
				diam_just = diam_path;

			} else {
				diam_path = 0;
				far_info[0] = just_far = info(0);
				for (int i = 0; i < 2; i++) {
					if (c[i]) {
						diam_path = max(diam_path, c[i]->far_info[0].far[0] + far_info[0].far[0]);
						diam_path = max(diam_path, c[i]->diam_path);
						far_info[0] = far_info[0] + c[i]->far_info[0];
						just_far = just_far + c[i]->far_info[0];
					}
				}
				just_far.far[1] = -1e18;
				far_info[1] = far_info[0];
				diam_just = diam_path;
			}
		}
	}
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		ptr[i] = new node();
		ptr[i]->is_vert = ptr[i]->is_path = true;
		ptr[i]->update();
	}
	for (int i = 0; i < n - 1; i++) {
		int u, v, w;
	...
	while (q--) {
		...
		u--;
		v--;
		auto gp = get_path(ptr[u], ptr[v]);
		lint a0 = gp->just_far.far[0];
		lint a1 = gp->just_far.far[0] + gp->just_far.far[1];
		lint a2 = gp->diam_just;
		cout << max({a0, a1, a2}) << "\n";
	}
}