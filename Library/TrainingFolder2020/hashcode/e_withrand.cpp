...
int extract_idx(int dates) {
  ...
  mt19937 mt(seed);
  double rnd = uniform_real_distribution<double>(0.85, 0.95)(mt);
	...
	for (int i = 0; i<L; i++) {
		...
		ret = max(ret, make_pair(1.0 * accm / pow(a[i].signup, rnd), i));
	}
	return ret.second;
}
int main() {
	...
	int score = -MCMF::match(0, sz(v) + B + 1, sz(v) + B + 2);
	for(int i=0; i<sz(v); i++){
		vector<int> novo;
	...
	if(score > 5213000){
  freopen(("./koo/" + to_string(score)).c_str(), "w", stdout);
	buf.flush();
}
  cerr << score << '\n';
}
