...
int main() {
    ...
    auto solve = [&](lint x) {
        int m = min(x - t, p);
        return ipow(mint(p + w - x + t + 1), m) * fact<mint>(p + w - m) * rfact<mint>(w);
    };
    mint ans = fact<mint>(e + p + w) * rfact<mint>(e) * rfact<mint>(w);
    for (int i = t; i <= p + w; i++) {
        ans -= (solve(i) - solve(i + 1)) * binom(e + p + w - i, e);
    }
    ans /= fact<mint>(e + p + w) * rfact<mint>(e) * rfact<mint>(w);
    cout << ans << "\n";
}