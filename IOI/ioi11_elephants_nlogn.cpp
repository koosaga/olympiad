...
struct elem {
    ...
    int mode; // 0->out, 1->in
    node *vtx, *edg;
    bool operator<(const elem &nd) const {
        if (key != nd.key)
            return key < nd.key;
        return mode < nd.mode;
    }
...
void insert(int x) {
    ...
    node *E = new node();
    E->sum = E->weight = 1;
    v1->is_path = v1->is_vert = true;
    v2->is_path = v2->is_vert = true;
    elem N1 = {x, 1, v1, NULL};
    elem N2 = {x + L, 0, v2, NULL};
    link(E, v1, v2);
    edges[x] = E;
    {
        auto it = s.lower_bound(N1);
        if (it != s.begin() && prev(it)->mode == 0) {
            it--;
            auto z = *it;
            s.erase(it);
            if (z.edg)
                cut(z.edg);
            z.edg = new node();
            link(z.edg, z.vtx, N1.vtx);
            s.insert(z);
        }
        s.insert(N1);
    }
    {
        auto it = s.lower_bound(N2);
        if (it != s.begin() && prev(it)->mode == 0) {
            it--;
            auto z = *it;
            s.erase(it);
            if (z.edg)
                cut(z.edg);
            z.edg = new node();
            link(z.edg, z.vtx, N2.vtx);
            s.insert(z);
        }
        it = s.lower_bound(N2);
        if (it != s.end()) {
            N2.edg = new node();
            link(N2.edg, N2.vtx, it->vtx);
        }
        s.insert(N2);
    }
}
...
void remove(int x) {
    cut(edges[x]);
    edges.erase(x);
    elem N1 = {x, 1, NULL, NULL};
    elem N2 = {x + L, 0, NULL, NULL};
    {
        auto it = s.find(N2);
        if (it->edg)
            cut(it->edg);
...
int query() {
    if (sz(s) == 0)
        return 0;
    return get_path(s.begin()->vtx, s.rbegin()->vtx)->sum;
}
int *A;
map<int, int> mp;
