...
struct node {
	polyn A[3][3];
	node() {}
	node(int x) { A[x][x] = polyn({mint(0), mint(1)}); }
		...
		for (int i = 0; i < 3; i++) {
			for (int j = i; j < 3; j++) {
				...
				for (int k = i; k <= j; k++)
					sum += A[i][k];
				for (int k = j; k < 3; k++) {
					ret.A[i][k] += sum * nd.A[j][k];
				}
			}
		}
		for (int i = 0; i < 3; i++) {
			for (int j = i; j < 3; j++) {
				ret.A[i][j] += A[i][j] + nd.A[i][j];
			}
		}
		return ret;
	}
...
mint binom(int u, int v) { return fact<mint>(u) * rfact<mint>(v) * rfact<mint>(u - v); }
...
int main() {
	...
	auto ans = solve(0, sz(a));
	mint dap = sz(a);
	for (int i = 1; i <= sz(a); i++) {
		for (int j = 0; j < 3; j++) {
			for (int k = j; k < 3; k++) {
				dap -= ans.A[j][k][i] / binom(sz(a), i);
			}
		}
	}
	cout << dap << "\n";
