...
int GB(int x, int y) { return (x >> y) & 1; }
struct node {
	array<lint, 3> op[16]; // which endstate it leads, with what cost for T = 1, T = 2
	node() {
		for (int i = 0; i < 16; i++)
			op[i][0] = i, op[i][1] = op[i][2] = 0;
	}
	node(char c, int R, int B) {
		for (int i = 0; i < 16; i++) {
			int T0Blue = (c == 'B');
			int T1Blue = GB(i, T0Blue);
			int T2Blue = GB(i, T1Blue + 2);
			op[i][0] = i;
			op[i][0] ^= (1 << T0Blue);
			op[i][0] ^= (1 << (T1Blue + 2));
			op[i][1] = (T1Blue ? B : R);
	...
	node operator+(const node &nd) const {
		node ret;
		for (int i = 0; i < 16; i++) {
			ret.op[i][0] = nd.op[op[i][0]][0];
			ret.op[i][1] = op[i][1] + nd.op[op[i][0]][1];
			ret.op[i][2] = op[i][2] + nd.op[op[i][0]][2];
		}
		return ret;
	}
};
...
int main() {
	...
	vector<node> seg(n);
	for (int i = 0; i < n; i++) {
		seg[i] = node(s[i], cost[i][0], cost[i][1]);
	}
	auto T = atcoder::lazy_segtree<node, TT, ET, int, UT, UU, EU>(seg);
	int q;
	cin >> q;
	while (q--) {
		int t;
