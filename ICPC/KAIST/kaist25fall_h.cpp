...
struct node : public top_tree_node_base<node> {
	...
	void update() {
		maxPath = pi{lint(-1e18), -1};
		if (is_path && !is_vert) {
			maxPath = max({c[0]->maxPath, c[1]->maxPath, pi{weight, index}});
		}
	}
};

vector<lint> solve(vector<array<lint, 3>> T1, vector<array<lint, 3>> T2) {
	int n = sz(T1) + 1;
	sort(all(T1));
	sort(all(T2));
	vector<node *> ptr(n);
	...
	for (int i = 0; i < n - 1; i++) {
		edges[i] = new node();
		edges[i]->weight = T2[i][0];
		edges[i]->index = i;
		edges[i]->update_all();
		link(edges[i], ptr[T2[i][1]], ptr[T2[i][2]]);
	}
	vector<lint> v;
	// find minimum lambda s.t T1 U (T2 + lambda) includes this edge.
	for (int i = 0; i < sz(T1); i++) {
		auto gp = get_path(ptr[T1[i][1]], ptr[T1[i][2]]);
		lint lambda = gp->maxPath[0] - T1[i][0];
		cut(edges[gp->maxPath[1]]);
		auto edge = new node();
		edge->weight = -lint(1e18);
		edge->index = -1;
		link(edge, ptr[T1[i][1]], ptr[T1[i][2]]);
		v.push_back(-lambda);
	}
	sort(all(v));
	lint sums = 0;
	for (auto &x : T2)
		sums += x[0];
	vector<lint> ans = {sums}; // 0 from T1
	for (int i = 0; i < n - 1; i++) {
		sums += v[i];
		ans.push_back(sums);
	}
