...
struct mincostflow{
    ...
    void add_edge(int s, int e, int x, InfInt c){
        gph[s].push_back({e, x, (int)gph[e].size(), c});
        gph[e].push_back({s, 0, (int)gph[s].size()-1, -c});
    }

    InfInt dist[MAXN];
    int pa[MAXN], pe[MAXN], chk[MAXN];
    bool inque[MAXN];
    ...
    bool spfa(int src, int sink){
        ...
        while(!que.empty()){
            ...
            for(int i=0; i<gph[x].size(); i++){
                edg e = gph[x][i];
                if(e.cap > 0 && (!chk[e.pos] || dist[e.pos] < dist[x] + e.cost)){
                    chk[e.pos] = 1;
    ...
    InfInt match(int src, int sink){
        InfInt ret = 0;
        while(spfa(src, sink)){
            ret += dist[sink];
            for(int pos = sink; pos != src; pos = pa[pos]){
                int rev = gph[pa[pos]][pe[pos]].rev;
                gph[pa[pos]][pe[pos]].cap--;
                gph[pos][rev].cap++;
            }
        }
        return ret;
...
int main(){
    ...
    for(int i=1; i<=n; i++){
        mcmf.add_edge(0, i, 1, (InfInt)0);
        mcmf.add_edge(i+n, 2*n+1, 1, (InfInt)0);
    }
    for(int i=0; i<n; i++){
        char buf[52];
        cin >> buf;
        for(int j=0; j<n; j++){
            ...
            mcmf.add_edge(i+1, j+n+1, 1, cost[51-piv]);
        }
    }
    InfInt m = mcmf.match(0, 2*n+1);
    for(int i=0; i<52; i++){
        while(m >= cost[51 - i]){
            if(i < 26) putchar(i + 'A');
            else putchar(i - 26 + 'a');
